...
while read id
do
  rm -rf ${MARKETODATADIR}/landingpages/${id}/segments*
  echo "getting segmentation for landing page id ${id} request number $i ..."
  mkdir -p ${MARKETODATADIR}/landingpages/${id}
  echo "using request URL : ${MARKETORESTURL}/segmentation/${id}/segments.json?access_token=${access_token}"
  curl "${MARKETORESTURL}/segmentation/${id}/segments.json?access_token=${access_token}" -o ${MARKETODATADIR}/landingpages/${id}/segments_${id}.json -s
  if [ $(cat "${MARKETODATADIR}/landingpages/${id}/landingpage_${id}.json" | jq -r '.["success"]') = "true" ]
    then
      echo -e "\e[32mSUCCESS!\e[0m"
    else
      echo -e "\n\n\e[31mFailure!\e[0m\n\n"
      echo "Errors: $(cat "${MARKETODATADIR}/landingpages/${id}/segments_${id}.json" | jq -r '.["errors"]')"
  fi
